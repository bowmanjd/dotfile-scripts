...
# of suitability:
#
# iwr -useb https://raw.githubusercontent.com/bowmanjd/dotfile-scripts/main/modulear-branching.ps1 | iex
#
...
function dtfrestore {
  Param ([string]$Module, [string]$Repo)
  git clone --single-branch -b $Module --bare $Repo "$dotfiles/MODULE"
  dtf $Module config --local status.showUntrackedFiles no
  dtf $Module checkout
  if ($LASTEXITCODE) {
